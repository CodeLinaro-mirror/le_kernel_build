...
def _create_makefiles_artifact_test(
    ...
    for (suffix, expected_lines) in (
        ("cflags", expected_cflags_lines),
        ("asflags", expected_asflags_lines),
        ("ldflags", expected_ldflags_lines),
    ):
        ...
        contain_lines_test(
            name = "{}_{}_test".format(name, suffix),
            expected = "{}_expected_{}".format(name, suffix),
            actual = "{}_{}".format(name, suffix),
            order = True,
        )
        tests.append("{}_{}_test".format(name, suffix))
...
def makefiles_test_suite(name):
    ...
    makefiles(
        ...
        internal_target_fail_message = "Invalid copts: -include $(location dep.h). $(location) expressions must be its own token, or part of -key=$(location target)",
        module_srcs = ["dep.h"],
        module_out = "dep.ko",
    ...
    makefiles(
        ...
        internal_target_fail_message = "Invalid copts: $(location dep.h) $(location dep.h). $(location) expressions must be its own token, or part of -key=$(location target)",
        module_srcs = ["dep.h"],
        module_out = "dep.ko",
